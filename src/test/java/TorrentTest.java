...
public class TorrentTest {
    ...
    public void setUp() throws Exception {
        ...
        client1 = new TorrentClient(trackerAddress, 0, folder1.getPath());
        folder2 = tempFolder.newFolder();
        client2 = new TorrentClient(trackerAddress, 0, folder2.getPath());
    }
    ...
    public void testRandom() throws Exception {
        for (int i = 0; i < RANDOM_TESTS; i++) {
            testLoadFile(random.nextInt(TorrentFile.PART_SIZE * 5));
        }
    }
    ...
    public void testMultipleClients() throws Exception {
        ...
        for (int i = 0; i < NUM_CLIENTS; i++) {
            ...
            folders.add(folder);
            clients.add(new TorrentClient(trackerAddress, 0, folder.getPath()));
        }

        String name = new BigInteger(100, random).toString(32);
        File file = new File(folders.get(0), name);
        String content = new BigInteger(TorrentFile.PART_SIZE + 1, random).toString(32);
    ...
    private void testLoadFile(int length) throws Exception {
        ...
        String content = new BigInteger(length * 5, random).toString(32);
        FileUtils.writeStringToFile(file, content, Charset.defaultCharset());
